...
int	interpolate_color(int color1, int color2, float factor)
{
	...
	return ((c.interpolated_red << 16)
		| (c.interpolated_green << 8) | c.interpolated_blue);
}
...
static void	color_management(t_data *data, t_color_management *t, t_line line)
{
	...
	t->initial_color = ft_rgb_to_int(calc_down_color(data, line.z0)
			+ data->map.extra_color, calc_up_color(data, line.z0), 250
			- data->map.extra_color);
	t->final_color = ft_rgb_to_int(calc_down_color(data, line.z1)
			+ data->map.extra_color, calc_up_color(data, line.z1), 250
			- data->map.extra_color);
	t->interpolated_color = interpolate_color(t->initial_color, t->final_color,
			t->interpolation_factor);
}

static void	line_management(t_data *data, t_line *line, int *z, t_rl *rl)
{
	rl->e2 = 2 * rl->err;
	if (rl->e2 > -rl->dy)
	{
...
static void	render_p_line_2(t_data *data, t_line line, t_rl rl, int z)
{
	...
	while (1)
	{
		...
		if (line.x0 == line.x1 && line.y0 == line.y1)
			break ;
		line_management(data, &line, &z, &rl);
		t.step++;
	}
}
