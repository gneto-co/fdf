...
/*   Updated: 2024/01/26 16:12:52 by gneto-co         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	line_1(t_data *data)
{
	...
	render_rect(data, (t_rect){0, WINDOW_HEIGHT / 2, WINDOW_WIDTH, 1,
		GREEN_PIXEL});
	render_rect(data, (t_rect){0, (WINDOW_HEIGHT / 2 + WINDOW_HEIGHT / 4),
		WINDOW_WIDTH, 1, WHITE_PIXEL});
	render_rect(data, (t_rect){WINDOW_WIDTH / 4, 0, 1, WINDOW_HEIGHT,
		WHITE_PIXEL});
	render_rect(data, (t_rect){WINDOW_WIDTH / 2, 0, 1, WINDOW_HEIGHT,
		RED_PIXEL});
	render_rect(data, (t_rect){(WINDOW_WIDTH / 2 + WINDOW_WIDTH / 4), 0, 1,
		WINDOW_HEIGHT, WHITE_PIXEL});
...
int	line_2(t_data *data)
{
	...
	while (i < x_number)
	{
		render_rect(data, (t_rect){0, ((WINDOW_HEIGHT / (x_number + 1)) * (i + 1)),
		 WINDOW_WIDTH, 1, WHITE_PIXEL});
		i++;
	}
	...
	while (i < y_number)
	{
		render_rect(data, (t_rect){((WINDOW_WIDTH / (y_number + 1)) * (i + 1)), 0,
		 1, WINDOW_HEIGHT, WHITE_PIXEL});
		i++;
	}
